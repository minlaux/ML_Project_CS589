...
In our implementation of the k-NN algorithm, we tested different values of $k$ in the range [1, 52) for an ideal hyperparameter. 
...
Additionally, a higher neuron count fascilitates learning these handwriting variations of numbers of the same class, making the network more reliable in predicting instances that look alike and a higher accuracy.
...
\subsection*{Performance Metrics}
One hot coded, loan ID was dropped
\newgeometry{left=1cm,right=1cm}
\begin{center}
    \texttt{Decision Tree}
\end{center}
